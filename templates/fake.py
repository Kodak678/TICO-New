...
def SaveBoardData(Boardsnaps,Boardscores):
    numpy.save("HugeBoards.npy",Boardsnaps, True)
    numpy.save("HugeScores.npy",Boardscores, True)
...
def create_model2():
    model = tf.keras.models.Sequential([tf.keras.layers.Dense(256,input_shape = (14,8,8), activation= tf.keras.activations.linear), 
...
# for i in range(0,100):  #All player's weights should be initialized with some exposure to some data    
...
# x = numpy.load("HugeBoards.npy")
# y = numpy.load("HugeScores.npy")
...
[0.35233160853385925, 0.2666666805744171]
# x = numpy.load("SumBoards.npy")
# y = numpy.load("SumScores.npy")
# y = numpy.asarray(y / abs(y).max() /2 ,dtype=numpy.float32) # normalization (0 - 1)
# model.fit(x, y,
#           batch_size=2048,
#           epochs=100,
#           verbose=1,
...
#           callbacks=[tf.keras.callbacks.ReduceLROnPlateau(monitor='accuracy', patience=5),
#                      tf.keras.callbacks.EarlyStopping(monitor='accuracy', patience=10, min_delta=1e-4)])
...
#           verbose=1,
#           validation_split=0.1,
#           callbacks=[tf.keras.callbacks.ReduceLROnPlateau(monitor='loss', patience=1),
#                      tf.keras.callbacks.EarlyStopping(monitor='loss', patience=1, min_delta=1e-4)])
...
# 0.0005861464305780828  (5.8615e-04)
